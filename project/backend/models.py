...
def init_db():
    ...
    if not LookupModels.query.first():
        ...
        db.session.add(dt)
        rf = LookupModels(name='Random Forest', description='Ensemble method combining multiple decision trees to improve accuracy.')
        db.session.add(rf)
        lr = LookupModels(name='Linear Regression', description='Models the relationship between dependent and independent variables using a linear equation.')
        db.session.add(lr)
        ...
        db.session.add(svm)
        knn = LookupModels(name='K-Nearest Neighbors', description='Classifies data based on the "k" closest training examples.')
        db.session.add(knn)
